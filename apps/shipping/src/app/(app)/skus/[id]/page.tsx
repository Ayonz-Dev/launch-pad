...
import { getSku } from "@/lib/data";
import { aglHref, containerHref, shipmentHref } from "@/lib/routes";
...
export default async function SkuDetail({ params }: { params: { id: string } }) {
  const item = await getSku(params.id);
  if (!item) notFound();

  return (
    <div className="mx-auto max-w-6xl px-5 py-6 lg:px-8">
      <Link href="/skus" className="text-sm text-slate-500 hover:text-harbor">
        ← SKUs
      </Link>

      <header className="mt-3">
        <p className="kicker mb-2">
          <span className="kicker-accent">Model</span>
        </p>
        <h1 className="tnum font-display text-2xl font-semibold text-ink">{item.model}</h1>
        ...
        <div className="overflow-hidden rounded-xl border border-slate-200 bg-white lg:col-span-2">
          <div className="border-b border-slate-100 px-4 py-3">
            ...
            <p className="mt-0.5 text-xs text-slate-500">
              Every shipment and container carrying this model.
            </p>
          </div>
            ...
            <table className="w-full text-left text-sm">
              <thead>
                <tr className="border-b border-slate-100 text-xs uppercase tracking-wide text-slate-500">
                  ...
                  <th className="px-4 py-2.5 font-medium">AGL</th>
              ...
              <tbody>
                {item.occurrences.map(({ shipment, line }) => (
                  <tr
                    key={`${shipment.id}-${line.agl ?? line.sku}`}
